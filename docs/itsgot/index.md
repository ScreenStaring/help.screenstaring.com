---
title: It's Got User's Guide
sidebar_position: 0
description: How to use the It's Got Nutrition Facts app for Shopify.
---

[It's Got](https://apps.shopify.com/nutrition-facts) manages product nutrition data, and allows one to
add supplement, drug, pet, and nutrition information panels to their site.
Labels can be generated in well-known formats for multiple countries & languages.

- [Adding Nutrition Labels to Your Site](/docs/itsgot/adding-labels)
- [Changing a Label's Colors and Design](/docs/itsgot/display-settings)
- [Linking to Web Pages from Label Nutrients](/docs/itsgot/display-settings-links)
- [Translating Label Text](/docs/itsgot/translations)
- [Importing Products in Bulk](/docs/itsgot/importing-products)
- [Creating Labels in Multiple Languages](/docs/itsgot/multi-language)
- [Combining Meal Components to Generate a Label](/docs/itsgot/combine-api)
- [Using Structured Data for Improved SEO](/docs/itsgot/structured-data)
- [Review Request Email Domain Verification](/docs/itsgot/review-request-email)
- [API Integration](/docs/itsgot/api-integration)
- [Uninstalling the App](/docs/itsgot/uninstalling)
- [Frequently Asked Questions](/docs/itsgot/faq)
